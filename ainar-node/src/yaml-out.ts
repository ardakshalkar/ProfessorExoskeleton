...
const resolvesAsNonString = (text: string): boolean =>
  RESOLVERS.some((pattern) => pattern.test(text));

// --------------------------------------------------------------------------
// Scalar style — `Emitter.analyze_scalar`, reduced to block context
// --------------------------------------------------------------------------

const allowsPlain = (text: string): boolean => {
  if (text === "") return false;
  if (/^\s|\s$/.test(text)) return false; // leading or trailing whitespace
  if (/[\0-\x08\x0b\x0c\x0e-\x1f\x7f-\x84\x86-\x9f]/.test(text)) return false;
  if (/[\n\x85\u2028\u2029]/.test(text)) return false; // folds as a quoted scalar
  if (/^[-?:,[\]{}#&*!|>'"%@`]/.test(text)) return false; // indicator first
 ...
 * one `\n` into a blank line.
 */
const write = (text: string, column: number, indent: string, quoted: boolean): string => {
  let out = "";
  let col = column;
  let spaces = false;
  let breaks = false;
  ...
  while (end <= text.length) {
    ...
    if (spaces) {
      ...
      if (ch === null || !BREAK.test(ch)) {
        if (quoted && BREAK.test(text[start]!)) out += "\n";
        for (const _ of text.slice(start, end)) out += "\n";
        out += indent;
        col = indent.length;
        start = end;
      }
...
const numberText = (value: number, isFloat: boolean): string => {
  if (Number.isNaN(value)) return ".nan";
  if (!Number.isFinite(value)) return value > 0 ? ".inf" : "-.inf";
  if (isFloat && Number.isInteger(value)) return `${value}.0`;
  return String(value);
};
 ...
 */
const scalar = (value: unknown, column: number, indent: string, isFloat: boolean): string => {
  if (value === null || value === undefined) return " null\n";
  if (typeof value === "boolean") return value ? " true\n" : " false\n";
  if (typeof value === "number") return " " + numberText(value, isFloat) + "\n";
...
const key = (name: string): string =>
  allowsPlain(name) && !resolvesAsNonString(name) ? name : "'" + name.replace(/'/g, "''") + "'";
...
export const dump = (value: unknown, isFloat: IsFloat = () => false): string => {
