 ...
 * `test/markdown.test.ts` pins both.
 */
 ...
 */
export const escapeAttribute = (value: unknown): string => escapeText(value).replace(/"/g, "&quot;");
 ...
 */
export const markdownHref = (value: unknown): string | null => {
...
export const markdownInline = (text: string): string => {
  const code: string[] = [];
  const html = escapeText(text)
    .replace(/`([^`\n]+)`/g, (_whole: string, body: string) => {
      code.push(body);
  ...
  return html.replace(
    /\u0000(\d+)\u0000/g,
    (_whole: string, index: string) => "<code>" + code[Number(index)] + "</code>",
  );
};

/** The line opens a block, and so ends the paragraph or item above it. */
const startsMarkdownBlock = (line: string): boolean =>
  line.trim() === "" ||
  /^```/.test(line) ||
  /^#{1,6}\s/.test(line) ||
  /^>/.test(line) ||
 ...
 */
export const renderMarkdown = (source: unknown): string => {
  ...
  while (index < lines.length) {
    ...
    if (/^\s*\|/.test(line) && /^\s*\|?[\s:|-]+\|[\s:|-]*$/.test(lines[index + 1] ?? "")) {
      ...
      out.push(
        "<table><thead><tr>" +
          head.map((cell: string) => "<th>" + markdownInline(cell) + "</th>").join("") +
          "</tr></thead><tbody>" +
          body
            .map(
              (row: string[]) =>
                ...
                head
                  .map((_column: string, at: number) => "<td>" + markdownInline(row[at] ?? "") + "</td>")
                  .join("") +
                "</tr>",
            )
            .join("") +
