 ...
 */
export const workspaceRootFor = (from: string | undefined | null): string | null => {
  if (!from) return null;
  let here = resolve(from);
  for (;;) {
    try {
      if (statSync(join(here, "courses")).isDirectory()) return here;
    } catch {
...
export class Workspace {
  ...
  constructor(source: string | CourseStore, origin: WorkspaceOrigin = "unknown") {
  ...
  findRun(courseVersionId: string): CourseBundle {
    for (const courseId of this.courseIds()) {
      try {
        ...
        if (runById(bundle!).has(courseVersionId)) return bundle!;
      } catch {
        continue;
      }
    }
...
export const requireRunId = (args: Record<string, unknown>): string => {
  const value = String(args.course_version_id ?? "").trim();
  if (!value) throw new ToolError("course_version_id is required, e.g. 'CSS-4008-2026-FALL'");
  return value;
};
...
export const referenceDate = (
  ...
  const run = runById(bundle).get(courseVersionId) as any;
