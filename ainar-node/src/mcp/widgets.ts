...
const read = (name: string): string =>
  readFileSync(join(ASSETS, name), "utf8").replace(/\r\n/g, "\n");
...
interface ManifestEntry {
  ...
  description: string;
  view: string;
  /** The markup as a template file, for a view that has been split into one. */
  structure?: string;
  invoking: string;
  invoked: string;
...
const build = (entry: ManifestEntry): Widget => {
...
export const WIDGETS: Widget[] = MANIFEST.widgets.map(build);
