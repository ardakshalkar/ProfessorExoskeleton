...
export const TELEGRAM_TOOL = {
  ...
  description:
    "Preview or publish one plain-text course announcement to the Telegram channel configured for a course run. Publishing is student-visible and requires explicit professor confirmation in the current request.",
  inputSchema: {
...
const result = (payload: unknown): ToolResult => ({
  content: [{ type: "text", text: JSON.stringify(payload, null, 2) }],
  structuredContent: payload,
});

export const callTelegramTool = async (
  publisher: TelegramPublisher,
  authInfo: AuthInfo | undefined,
  args: Record<string, unknown>,
): Promise<ToolResult> => {
  if (!authInfo) throw new Error("Telegram publishing requires OAuth");
  ...
  if (message.length > 4096) throw new Error("Telegram messages may not exceed 4096 characters");
  const identity: OAuthIdentity = {
    issuer: String(authInfo.extra?.issuer ?? ""),
    subject: String(authInfo.extra?.subject ?? ""),
  };
  if (!identity.issuer || !identity.subject) throw new Error("OAuth identity is incomplete");
