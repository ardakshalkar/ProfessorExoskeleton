...
const assessmentEntry = (assessment: any, rubrics: Map<string, any>): Record<string, unknown> => {
  ...
  return {
    ...
    //
    // The outline used to carry every fact ABOUT a piece of graded work — its
    // weight, its dates, whether a rubric exists — and not one word of the work
    // itself, on the reasonable ground that a term plan is a shape rather than a
    // reader. But an assessment in this model usually has no
    // `instructions_document_id`: the brief lives in the record's own
    // `description`, and a view with nothing to open therefore had nothing to
    // show at all. Naming the file would not have helped, because for most
    // courses there is no file.
    //
    ...
    // reaches a document as words rather than as markup — which matters, since
    // the same payload serves the public course page.
