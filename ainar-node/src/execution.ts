...
export type ExecutionPlan =
  | { available: true; target: ExecutionTarget; action: ActionPolicy }
  | { available: false; action: ActionPolicy; reason: string };

const policy = (
  name: string,
  values: Omit<ActionPolicy, "name">,
): ActionPolicy => ({ name, ...values });
 ...
 */
export const ACTIONS: readonly ActionPolicy[] = [
  ...[
    "syllabus-review",
    "course-reflection",
    "onboard-course",
...
export const resolveExecution = (
  ...
  if (!action) throw new Error(`unknown Professor Harness action '${name}'`);
