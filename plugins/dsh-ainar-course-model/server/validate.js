 ...
 */
import { allRubrics, assessmentById, assessmentsOf, capabilityById, conceptById, criterionById, documentById, itemById, moduleById, modulesOf, outcomesOf, resourceById, itemModelById, itemsOf, outcomeById, runById, userById, } from "./bundle.js";
import { statSync } from "node:fs";
import { join } from "node:path";
import { IssueList } from "./issues.js";
/** Whether a repository-relative storage key names an existing file. */
const isFile = (root, key) => {
    try {
...
export const coverage = () => ({
    ...
    total: TOTAL_CODES,
    note: `${IMPLEMENTED.length} of ${TOTAL_CODES} checks` +
...
const index = (records, field) => new Map(records.map((record) => [record[field], record]));
const ID_FIELDS = [
    ["outcomes", "outcome_id"],
    ["concepts", "concept_id"],
...
const checkDuplicates = (b, issues) => {
    for (const [collection, field] of ID_FIELDS) {
        const seen = new Set();
        for (const record of b[collection]) {
...
const checkConcepts = (b, issues) => {
    const known = conceptById(b);
    for (const concept of b.concepts) {
        for (const neighbour of concept.prerequisites) {
        ...
        for (const neighbour of concept.related) {
            if (!known.has(neighbour)) {
    ...
    const seen = new Map();
    for (const concept of b.concepts) {
        for (const name of [concept.title, ...concept.aliases]) {
            const key = String(name).toLowerCase().split(/\s+/).join(" ");
            const first = seen.get(key);
            if (first === undefined)
                seen.set(key, concept.concept_id);
            else if (first !== concept.concept_id) {
                issues.warn("concept.duplicate_name", `'${name}' already names ${first}; if they are the same concept, merge them and keep one identifier`, concept.concept_id);
            }
        }
    }
...
const checkCycles = (b, issues) => {
    const graph = new Map(b.concepts.map((c) => [c.concept_id, []]));
    for (const concept of b.concepts) {
        for (const prerequisite of concept.prerequisites) {
            graph.get(prerequisite)?.push(concept.concept_id);
        }
    }
    for (const edge of b.concept_edges) {
 ...
 * check exists for.
 */
const checkPresentationPlans = (b, issues) => {
    ...
    for (const document of b.documents) {
        ...
        if (plan.max_slides != null && plan.slides.length > plan.max_slides) {
            issues.warn("presentation.slide_limit", `plan contains ${plan.slides.length} slides but max_slides is ${plan.max_slides}`, where);
        }
        const planned = plan.slides.reduce((total, slide) => total + (slide.minutes ?? 0), 0);
        if (plan.duration_minutes != null && planned) {
            if (Math.abs(planned - plan.duration_minutes) > Math.max(5, plan.duration_minutes * 0.1)) {
                issues.warn("presentation.duration", `slide timings total ${g(planned)} minutes but deck duration is ${plan.duration_minutes}`, where);
            }
        }
...
const checkAssessments = (b, issues) => {
    ...
    for (const assessment of b.assessments) {
        ...
        if (run === undefined) {
            issues.error("ref.course_run", `unknown course_version_id ${assessment.course_version_id}`, assessment.assessment_id);
            continue;
        }
        for (const outcomeId of assessment.outcomes ?? []) {
            if (!outcomes.has(outcomeId)) {
        ...
        if (assessment.due_at) {
            ...
            if (!(run.start_date <= due && due <= run.end_date)) {
                issues.warn("schedule.outside_run", `due ${due}, outside the run window`, assessment.assessment_id);
            }
        }
        ...
        if (!(assessment.submission_type ?? []).length) {
        ...
        if (design != null) {
            const declared = new Set(assessment.outcomes ?? []);
            for (const cell of design.cells ?? []) {
                if (!outcomes.has(cell.outcome_id)) {
                    issues.error("ref.outcome", `blueprint cell names unknown outcome ${cell.outcome_id}`, assessment.assessment_id);
                }
                else if (!declared.has(cell.outcome_id)) {
                    issues.warn("assessment.design_scope", `blueprint cell names ${cell.outcome_id}, which the assessment does not declare`, assessment.assessment_id);
                }
            }
            if ((design.cells ?? []).length) {
                const marks = design.cells.reduce((sum, cell) => sum + cell.marks, 0);
                if (Math.abs(marks - assessment.maximum_score) > 0.01) {
                    issues.warn("assessment.design_marks", `blueprint cells allocate ${g(marks)} marks but maximum_score is ` +
                        `${g(assessment.maximum_score)}`, assessment.assessment_id);
                }
                const count = design.cells.reduce((sum, cell) => sum + cell.item_count, 0);
                if (design.target_item_count != null && count !== design.target_item_count) {
                    issues.warn("assessment.design_count", `blueprint cells request ${count} items but target_item_count is ` +
                        `${design.target_item_count}`, assessment.assessment_id);
                }
            }
        }
        ...
        if (assessment.rubric_id && rubric === undefined) {
            ...
            issues.warn("assessment.no_rubric", "no rubric — criterion-level grading and evidence are not possible", assessment.assessment_id);
        }
        else if ((rubric.criteria ?? []).length) {
            const total = rubric.criteria.reduce((sum, criterion) => sum + criterion.maximum_score, 0);
            if (Math.abs(total - assessment.maximum_score) > 0.01) {
                issues.warn("rubric.score_mismatch", `criteria sum to ${g(total)} but maximum_score is ${g(assessment.maximum_score)}`, assessment.assessment_id);
            }
        }
    }
    for (const rubric of rubrics.values()) {
        ...
        for (const criterion of rubric.criteria ?? []) {
            if (criterion.rubric_id !== rubric.rubric_id) {
                issues.error("ref.rubric", `criterion declares rubric_id ${criterion.rubric_id}, found in ${rubric.rubric_id}`, criterion.criterion_id);
            }
            if (criterion.outcome_id == null) {
                issues.warn("criterion.no_outcome", "criterion is not linked to a learning outcome, so it produces marks but no evidence", criterion.criterion_id);
            }
            ...
            if (criterion.capability_id && !capabilities.has(criterion.capability_id)) {
                issues.error("ref.capability", `unknown capability ${criterion.capability_id}`, criterion.criterion_id);
            }
    ...
    for (const run of b.versions) {
        ...
        if (weighted.length && weighted.length === assessments.length) {
            ...
            if (Math.abs(total - 1.0) > WEIGHT_TOLERANCE) {
                issues.warn("weight.sum", `assessment weights sum to ${total.toFixed(3)}, expected 1.0`, run.course_version_id);
            }
        }
        else if (weighted.length) {
            issues.warn("weight.partial", `${weighted.length} of ${assessments.length} assessments declare a weight`, run.course_version_id);
        }
    }
};
/**
 ...
 */
const checkVersions = (b, issues) => {
    const courseId = b.course.course_id;
    ...
    for (const version of b.versions) {
        if (version.course_id !== courseId) {
            issues.error("ref.course", `course_id ${version.course_id} does not match ${courseId}`, version.course_version_id);
        }
...
const checkConceptOwnership = (b, issues) => {
    const courseId = b.course.course_id;
    for (const concept of b.concepts) {
    ...
    for (const edge of b.concept_edges) {
        ...
        for (const endpoint of [edge.source_concept_id, edge.target_concept_id]) {
            if (!known.has(endpoint)) {
                issues.error("ref.concept", `unknown concept ${endpoint} in ${edge.relationship_type} edge`, `${edge.source_concept_id}->${edge.target_concept_id}`);
            }
        }
    }
};
const checkOutcomes = (b, issues) => {
    const courseId = b.course.course_id;
    const concepts = conceptById(b);
    const capabilities = capabilityById(b);
    for (const outcome of b.outcomes) {
        ...
        for (const conceptId of outcome.concepts ?? []) {
            if (!concepts.has(conceptId)) {
    ...
    for (const version of b.versions) {
        ...
        if (!outcomes.length) {
            issues.warn("version.no_outcomes", "course version has no learning outcomes", version.course_version_id);
            continue;
        }
        ...
        if (weighted.length && weighted.length === outcomes.length) {
            ...
            if (Math.abs(total - 1.0) > WEIGHT_TOLERANCE) {
                issues.warn("weight.sum", `outcome weights sum to ${total.toFixed(3)}, expected 1.0`, version.course_version_id);
            }
        }
        else if (weighted.length) {
            issues.warn("weight.partial", `${weighted.length} of ${outcomes.length} outcomes declare a weight`, version.course_version_id);
        }
    }
};
const checkModules = (b, issues) => {
    const courseId = b.course.course_id;
    const outcomes = outcomeById(b);
    const concepts = conceptById(b);
    for (const module of b.modules) {
        ...
        for (const outcomeId of module.outcomes ?? []) {
            ...
            if (outcome === undefined) {
                ...
                issues.error("ref.version_mismatch", `outcome ${outcomeId} belongs to ${outcome.course_id}`, module.module_id);
            }
        }
    ...
    for (const version of b.versions) {
        ...
        for (const duplicate of duplicates(weeks)) {
            issues.warn("module.week_clash", `more than one module is scheduled in week ${duplicate}`, version.course_version_id);
        }
    }
};
const checkRuns = (b, issues) => {
    const courseId = b.course.course_id;
    const runs = runById(b);
    const users = userById(b);
    for (const run of b.versions) {
        if (!run.course_version_id.startsWith(`${courseId}-`)) {
            issues.error("id.convention", `course_version_id must start with ${courseId}-`, run.course_version_id);
        }
        if (run.course_version_id !== `${courseId}-${run.term}`) {
            issues.error("id.convention", `course_version_id does not match term ${run.term}`, run.course_version_id);
        }
    ...
    for (const enrollment of b.enrollments) {
        if (!runs.has(enrollment.course_version_id)) {
            issues.error("ref.course_run", `unknown course_version_id ${enrollment.course_version_id}`, enrollment.enrollment_id);
        }
        const key = `${enrollment.course_version_id}\u0000${enrollment.student_id}`;
        if (seen.has(key)) {
            issues.error("enrollment.duplicate", `${enrollment.student_id} is enrolled twice in ${enrollment.course_version_id}`, enrollment.enrollment_id);
        }
        seen.add(key);
    }
};
 ...
 */
const checkStudentIdentifiers = (b, issues) => {
    const sources = [
        ...b.enrollments.map((e) => [e.student_id, e.enrollment_id]),
        ...b.submissions.map((s) => [s.student_id, s.submission_id]),
        ...b.item_responses.map((r) => [r.student_id, r.response_id]),
        ...b.evidence.map((e) => [e.student_id, e.evidence_id]),
        ...b.signals
            .filter((s) => s.student_id)
            .map((s) => [s.student_id, s.signal_id]),
        ...b.interventions
            .filter((i) => i.student_id)
            .map((i) => [i.student_id, i.intervention_id]),
        ...b.concept_states.map((c) => [c.student_id, `${c.student_id}/${c.concept_id}`]),
    ];
    const seen = new Set();
    for (const [studentId, where] of sources) {
        if (seen.has(studentId) || PSEUDONYM.test(studentId))
            continue;
        seen.add(studentId);
        issues.warn("privacy.identifier", `'${studentId}' does not look pseudonymous — student identifiers should come from ` +
            "`ainar roster import`, which keeps names outside this repository", where);
    }
};
/**
...
const checkActivities = (b, issues) => {
    const runs = runById(b);
    const modules = moduleById(b);
    const outcomes = outcomeById(b);
    const concepts = conceptById(b);
    ...
    for (const activity of b.activities) {
        ...
        if (run === undefined) {
            issues.error("ref.course_run", `unknown course_version_id ${activity.course_version_id}`, activity.activity_id);
            continue;
        }
        ...
        for (const conceptId of activity.concepts ?? []) {
            if (!concepts.has(conceptId)) {
        ...
        if (activity.module_id == null) {
            issues.warn("schedule.no_module", "meeting names no module, so nothing links it to what is taught " +
                "(deliberate for a revision or consultation slot)", activity.activity_id);
        }
        if (activity.scheduled_at) {
            const scheduled = String(activity.scheduled_at).slice(0, 10);
            if (!(run.start_date <= scheduled && scheduled <= run.end_date)) {
                issues.warn("schedule.outside_run", `scheduled ${scheduled}, outside ${run.start_date}..${run.end_date}`, activity.activity_id);
            }
            const slot = `${activity.course_version_id}\u0000${activity.scheduled_at}`;
            const clash = slots.get(slot);
            if (clash !== undefined) {
                issues.warn("schedule.slot_clash", `starts at the same moment as ${clash} in the same run`, activity.activity_id);
            }
            else {
            ...
            if (module !== undefined && module.week != null) {
                ...
                if (taught !== module.week) {
                    issues.warn("schedule.week_mismatch", `falls in week ${taught} of the run but teaches ${module.module_id}, which is ` +
                        `week ${module.week} (a holiday shifting the calendar is a fair reason)`, activity.activity_id);
                }
            }
        }
    }
...
const checkItemModels = (b, issues) => {
    const outcomes = outcomeById(b);
    const capabilities = capabilityById(b);
    const concepts = conceptById(b);
    const runs = runById(b);
    for (const model of b.item_models) {
        if (!runs.has(model.course_version_id)) {
            issues.error("ref.course_version", `unknown course_version_id ${model.course_version_id}`, model.item_model_id);
        }
        ...
        if (model.capability_id && !capabilities.has(model.capability_id)) {
            issues.error("ref.capability", `unknown capability ${model.capability_id}`, model.item_model_id);
        }
...
const checkItemIntent = (b, issues) => {
    ...
    for (const item of b.items) {
        if (item.maximum_score === 0 && item.role !== "preparation") {
            issues.warn("item.unmarked", `carries no marks but its role is '${item.role}', so it can never contribute to a ` +
                "score — set a maximum, or mark it as a preparation item if that is what it is", item.item_id);
        }
        const scores = observed.get(item.item_id) ?? [];
        if (item.difficulty == null || scores.length < DIFFICULTY_SAMPLE)
            continue;
        const rate = scores.reduce((sum, value) => sum + value, 0) / scores.length;
        if (item.difficulty === "easy" && rate < 0.5) {
            issues.warn("item.difficulty_mismatch", `declared easy but ${percent(rate)} of ${scores.length} responses scored — ` +
                "either the wording or the teaching, not the students", item.item_id);
        }
        else if (item.difficulty === "complex" && rate > 0.9) {
            issues.warn("item.difficulty_mismatch", `declared complex but ${percent(rate)} of ${scores.length} responses scored, ` +
                "so it is not discriminating between students", item.item_id);
        }
    }
};
 ...
 */
const checkDelivery = (b, issues) => {
    const canvasId = (assessment) => {
        ...
        for (const assessment of assessments) {
            ...
            if (channel === "paper_exam") {
                ...
                if (!questions || !questions.length) {
                    issues.warn("delivery.unreachable", "is sat on paper but has no extensions.paper.questions, so `exam-setup` and " +
                        "`import-graded-exam` cannot map a question to a criterion", where);
                }
            }
            if (channel === "oral_defense") {
                const files = (assessment.submission_type ?? [])
                    .filter((format) => FILE_FORMATS.has(format))
                    .sort();
                if (files.length) {
                    issues.warn("delivery.contradiction", `arrives as an oral defence but declares ${files.join(", ")} as a submission ` +
                        "format — one of the two is wrong", where);
                }
            }
        }
    }
 ...
 */
const checkDocuments = (b, issues, root) => {
    ...
    for (const document of b.documents) {
        if (keys.has(document.storage_key)) {
            issues.error("document.duplicate_key", `storage_key is already used by ${keys.get(document.storage_key)}`, document.document_id);
        }
        ...
        if (document.course_version_id && !runs.has(document.course_version_id)) {
            ...
            issues.error("ref.course_run", `unknown course_version_id ${document.course_version_id}`, document.document_id);
            issues.error("ref.course_version", `unknown course_version_id ${document.course_version_id}`, document.document_id);
        }
        ...
        for (const conceptId of document.concepts ?? []) {
            if (!concepts.has(conceptId)) {
        ...
        if (document.checksum && !String(document.checksum).startsWith("sha256:")) {
            issues.warn("document.checksum", "checksum should be prefixed with the algorithm, e.g. 'sha256:'", document.document_id);
        }
        if (root !== undefined && !String(document.storage_key).includes("://")) {
            if (!isFile(root, document.storage_key)) {
                issues.error("document.missing_file", `${document.storage_key} does not exist — a storage_key with no scheme is a path ` +
                    "in this repository; use 'object://…' for object storage", document.document_id);
            }
        }
    }
    ...
    for (const resource of b.resources) {
        if (resource.document_id)
            referenced.push([resource.document_id, resource.resource_id]);
    }
    ...
    for (const evaluation of b.evaluations) {
        if (evaluation.ai_suggestion) {
            for (const reference of evaluation.ai_suggestion.evidence ?? []) {
    ...
    for (const [documentId, where] of referenced) {
        if (!known.has(documentId)) {
            issues.warn("ref.document", `${documentId} is not registered — add it to documents.yaml, or leave it if the ` +
                "application owns the file", where);
        }
    }
};
const checkResources = (b, issues) => {
    const runs = runById(b);
    const concepts = conceptById(b);
    for (const resource of b.resources) {
        if (resource.course_version_id && !runs.has(resource.course_version_id)) {
            issues.error("ref.course_run", `unknown course_version_id ${resource.course_version_id}`, resource.resource_id);
            issues.error("ref.course_version", `unknown course_version_id ${resource.course_version_id}`, resource.resource_id);
        }
        ...
        if (resource.document_id == null && resource.url == null) {
            issues.warn("resource.no_location", "resource has neither a document_id nor a url", resource.resource_id);
        }
    }
};
/** The design chain: outcome -> concept -> activity -> criterion -> evidence. */
const checkCoverage = (b, issues) => {
    const assessedOutcomes = new Set();
    for (const rubric of allRubrics(b).values()) {
        for (const criterion of rubric.criteria ?? []) {
            if (criterion.outcome_id)
                assessedOutcomes.add(criterion.outcome_id);
        }
    }
    for (const assessment of b.assessments) {
        for (const outcomeId of assessment.outcomes ?? []) {
    ...
    for (const item of b.items) {
        if (item.outcome_id)
            assessedOutcomes.add(item.outcome_id);
    }
    const taughtOutcomes = new Set();
    for (const module of b.modules) {
        for (const outcomeId of module.outcomes ?? [])
            taughtOutcomes.add(outcomeId);
    }
    for (const activity of b.activities) {
        for (const outcomeId of activity.outcomes ?? [])
            taughtOutcomes.add(outcomeId);
    ...
    for (const outcome of b.outcomes) {
        if (!taughtOutcomes.has(outcome.outcome_id)) {
            issues.warn("coverage.untaught_outcome", "no module or activity addresses this outcome", outcome.outcome_id);
        }
        if (!assessedOutcomes.has(outcome.outcome_id)) {
            issues.warn("coverage.unassessed_outcome", "no assessment or rubric criterion measures this outcome", outcome.outcome_id);
        }
    }
    const taughtConcepts = new Set();
    for (const module of b.modules) {
        for (const conceptId of module.concepts ?? [])
            taughtConcepts.add(conceptId);
    }
    for (const activity of b.activities) {
        for (const conceptId of activity.concepts ?? [])
            taughtConcepts.add(conceptId);
    ...
    for (const concept of b.concepts) {
        if (!taughtConcepts.has(concept.concept_id)) {
            issues.warn("coverage.untaught_concept", "no module or activity teaches this concept", concept.concept_id);
        }
    }
    ...
    for (const rubric of allRubrics(b).values()) {
        for (const criterion of rubric.criteria ?? []) {
            if (criterion.capability_id)
                usedCapabilities.add(criterion.capability_id);
        }
    }
    for (const outcome of b.outcomes) {
        for (const capabilityId of outcome.capabilities ?? []) {
    ...
    for (const capability of b.capabilities) {
        if (!usedCapabilities.has(capability.capability_id)) {
            issues.warn("coverage.unused_capability", "no outcome or criterion contributes to this capability", capability.capability_id);
        }
    }
};
// --------------------------------------------------------------------------
 ...
 */
const defaultSheetTab = (assessment) => {
    const remainder = String(assessment.assessment_id).replace(/^ASSESSMENT-/, "");
    if (!remainder)
        return sanitiseTab(assessment.assessment_id);
    return sanitiseTab(/^\d/.test(remainder) ? `A${remainder}` : remainder);
};
const effectiveSheetTab = (assessment) => lmsString(assessment, SHEET_TAB_KEY) ?? defaultSheetTab(assessment);
const effectiveSummaryTab = (run) => lmsString(run, SHEET_TAB_KEY) ?? SUMMARY_TAB;
/** Which entities want which tab, defaults included. */
const tabOwners = (b, courseVersionId) => {
 ...
 */
const checkSheetLinks = (b, run, courseLink, issues) => {
    const spreadsheet = courseLink[SHEET_ID_KEY];
    if (spreadsheet != null &&
        (String(spreadsheet).includes("docs.google.com") || String(spreadsheet).startsWith("http"))) {
        issues.warn("lms.sheet_url", `${SHEET_ID_KEY} holds a URL; the id alone — the part between /d/ and /edit — is what ` +
            "the API wants and what survives a moved document", run.course_version_id);
    }
    ...
    const entities = [
        ...
        ...assessmentsOf(b, run.course_version_id).map((a) => [a, a.assessment_id]),
    ];
    for (const [entity, location] of entities) {
        ...
        if (bad.length) {
            issues.error("lms.malformed", `${SHEET_TAB_KEY} '${written}' contains ${bad.join(" ")}, which Google Sheets will ` +
                "not accept in a tab name", location);
        }
        else if (written.length > MAX_TAB_LENGTH) {
            issues.error("lms.malformed", `${SHEET_TAB_KEY} is ${written.length} characters; Google Sheets allows ${MAX_TAB_LENGTH}`, location);
        }
        ...
        if (spreadsheet == null && entity !== run) {
            issues.warn("lms.tab_without_sheet", `names a sheet tab '${written}' but ${run.course_version_id} has no ${SHEET_ID_KEY}, ` +
                "so there is no spreadsheet to write it into", location);
        }
    }
    ...
    for (const tab of [...owners.keys()].sort()) {
        ...
        if (claimants.length > 1) {
            issues.error("lms.duplicate_tab", `tab '${tab}' is claimed by ${claimants.join(", ")}; each push rewrites a tab whole, ` +
                `so one would erase the other. Set extensions.lms.${SHEET_TAB_KEY} on one of them`, run.course_version_id);
        }
    }
    ...
    if (spreadsheet != null) {
        ...
        for (const other of b.versions) {
            if (other.course_version_id >= run.course_version_id)
                continue; // report once, from the later run
            if (lmsString(other, SHEET_ID_KEY) !== String(spreadsheet))
                continue;
            const theirs = new Set(tabOwners(b, other.course_version_id).keys());
            const shared = [...ours].filter((tab) => theirs.has(tab)).sort();
            if (shared.length) {
                issues.error("lms.duplicate_tab", `shares spreadsheet ${spreadsheet} with ${other.course_version_id} and both write ` +
                    `${shared.join(", ")}. Use a spreadsheet per run, or set ` +
                    `extensions.lms.${SHEET_TAB_KEY} to tell them apart`, run.course_version_id);
            }
        }
    }
};
 ...
 */
const checkLmsLinks = (b, issues) => {
    const linkage = (entity, location) => {
        const found = (entity.extensions ?? {})[LMS_EXTENSION];
        if (found == null)
            return {};
        if (typeof found !== "object" || Array.isArray(found)) {
            issues.error("lms.malformed", `extensions.${LMS_EXTENSION} must be a mapping, not ${typeName(found)}`, location);
            return {};
        }
        return found;
    };
    for (const run of b.versions) {
        ...
        if (target != null && !TARGETS.includes(String(target))) {
            ...
            const extra = String(target) === "notion"
                ? " `ainar notion push` publishes course content to Notion, not grades."
                : "";
            issues.warn("lms.unsupported_target", `'${target}' is recorded as this run's target, and nothing here can push grades to ` +
                `it. Supported: ${TARGETS.join(", ")}. Export a file and upload it by hand until ` +
                `an adapter exists.${extra}`, run.course_version_id);
        }
        const courseId = courseLink[CANVAS_COURSE_KEY];
        if (courseId != null && !/^\d+$/.test(String(courseId))) {
            issues.error("lms.malformed", `${CANVAS_COURSE_KEY} should be the numeric Canvas course id, not '${courseId}'`, run.course_version_id);
        }
        const assessments = assessmentsOf(b, run.course_version_id);
        const seen = new Map();
        const linked = [];
        for (const assessment of assessments) {
            ...
            if (!/^\d+$/.test(String(assignmentId))) {
                issues.error("lms.malformed", `${CANVAS_ASSIGNMENT_KEY} should be the numeric Canvas assignment id, not ` +
                    `'${assignmentId}'`, assessment.assessment_id);
                continue;
            }
            linked.push(assessment.assessment_id);
            const owner = seen.get(String(assignmentId));
            if (owner !== undefined) {
                issues.error("lms.duplicate_link", `Canvas assignment ${assignmentId} is already claimed by ${owner}; two assessments ` +
                    "cannot share one gradebook column", assessment.assessment_id);
            }
            else {
        ...
        if (linked.length && linked.length < assessments.length) {
            const unlinked = assessments
                ...
                .sort();
            issues.warn("lms.partial_links", `${linked.length} of ${assessments.length} assessments are linked to Canvas; ` +
                `${unlinked.join(", ")} cannot be exported by id`, run.course_version_id);
        }
...
const normaliseNotionId = (value) => value.replace(/-/g, "").trim().toLowerCase();
/**
 ...
 */
const checkNotionLinks = (b, issues) => {
    for (const run of b.versions) {
        const found = (run.extensions ?? {})[NOTION_EXTENSION];
        if (found == null)
            continue;
        if (typeof found !== "object" || Array.isArray(found)) {
            issues.error("notion.malformed", `extensions.${NOTION_EXTENSION} must be a mapping, not ${typeName(found)}`, run.course_version_id);
            continue;
        }
        const mapping = found;
        const unknown = Object.keys(mapping)
            ...
            .sort();
        if (unknown.length) {
            issues.warn("notion.unknown_key", `nothing reads ${unknown.join(", ")}; expected ${[...NOTION_DATABASES].sort().join(", ")}`, run.course_version_id);
        }
        for (const key of NOTION_DATABASES) {
            ...
            if (asText.includes("notion.so") || asText.startsWith("http")) {
                issues.warn("notion.database_url", `${key} holds a URL; the id alone — the 32 characters before the '?v=' — is what ` +
                    "the API wants", run.course_version_id);
                continue;
            }
            const bare = normaliseNotionId(asText);
            if (bare.length !== 32 || ![...bare].every((char) => "0123456789abcdef".includes(char))) {
                issues.error("notion.malformed", `${key} should be a 32-character Notion id, not '${asText}'`, run.course_version_id);
            }
        }
        const missing = NOTION_DATABASES.filter((key) => !mapping[key]);
        if (missing.length && missing.length < NOTION_DATABASES.length) {
            issues.warn("notion.partial", `${[...missing].sort().join(", ")} not recorded; \`notion pull\` reads what is there ` +
                "and reports the rest as absent", run.course_version_id);
        }
    }
};
/**
...
const checkItems = (b, issues) => {
    ...
    for (const item of b.items) {
        ...
        if (item.item_model_id) {
            ...
            if (model === undefined) {
                ...
                if (model.course_version_id !== assessment.course_version_id) {
                    issues.error("item.model_mismatch", `item model belongs to ${model.course_version_id}, not ${assessment.course_version_id}`, item.item_id);
                }
                if ((model.allowed_item_types ?? []).length &&
                    !model.allowed_item_types.includes(item.type)) {
                    issues.warn("item.model_mismatch", `${item.type} is not allowed by ${item.item_model_id}`, item.item_id);
                }
                if (model.outcome_id && item.outcome_id !== model.outcome_id) {
                    issues.warn("item.model_mismatch", `item outcome ${item.outcome_id} differs from model outcome ${model.outcome_id}`, item.item_id);
                }
                const tagged = new Set(item.concepts ?? []);
                const missing = (model.concepts ?? []).filter((c) => !tagged.has(c)).sort();
                if (missing.length) {
                    issues.warn("item.model_mismatch", "item omits model concepts " + missing.join(", "), item.item_id);
                }
            }
        }
        ...
        if (item.criterion_id) {
            if (!criteria.has(item.criterion_id)) {
                ...
                const rubric = assessment.rubric_id ? rubrics.get(assessment.rubric_id) : undefined;
                const owned = new Set(rubric ? (rubric.criteria ?? []).map((c) => c.criterion_id) : []);
                if (!owned.has(item.criterion_id)) {
                    issues.error("ref.rubric", `criterion ${item.criterion_id} does not belong to the rubric of ${item.assessment_id}`, item.item_id);
                }
            }
        }
        ...
        for (const option of item.options ?? []) {
            ...
            if (target && !concepts.has(target)) {
                issues.error("ref.concept", `option '${option.label}' points at unknown concept ${target}`, item.item_id);
            }
        }
        if (!(item.concepts ?? []).length) {
            issues.warn("item.no_concepts", "item is not tagged with any concept, so it cannot inform diagnosis", item.item_id);
        }
    ...
    for (const assessment of b.assessments) {
        ...
        if (!items.length)
            continue;
        const total = items.reduce((sum, item) => sum + item.maximum_score, 0);
        if (Math.abs(total - assessment.maximum_score) > 0.01) {
            issues.warn("item.score_mismatch", `items sum to ${g(total)} but maximum_score is ${g(assessment.maximum_score)}`, assessment.assessment_id);
        }
        const numbers = items.filter((item) => item.number != null).map((item) => String(item.number));
        for (const duplicate of duplicates(numbers)) {
            issues.warn("item.number_clash", `more than one item is numbered ${duplicate}`, assessment.assessment_id);
        }
        ...
        for (const item of items) {
            if (item.criterion_id) {
                perCriterion.set(item.criterion_id, (perCriterion.get(item.criterion_id) ?? 0) + item.maximum_score);
            }
        }
        for (const criterionId of [...perCriterion.keys()].sort()) {
            ...
            if (Math.abs(itemTotal - criterion.maximum_score) > 0.01) {
                issues.warn("item.criterion_score_mismatch", `items feeding this criterion sum to ${g(itemTotal)} but its maximum_score is ` +
                    `${g(criterion.maximum_score)}, so an item-derived score would need rescaling`, criterionId);
            }
        }
    }
    const items = itemById(b);
    const submissions = index(b.submissions, "submission_id");
    for (const response of b.item_responses) {
        const item = items.get(response.item_id);
        ...
        if (submission === undefined) {
            issues.error("ref.submission", `unknown submission ${response.submission_id}`, response.response_id);
        }
        if (item !== undefined && submission !== undefined) {
            if (item.assessment_id !== submission.assessment_id) {
                issues.error("ref.assessment", `item belongs to ${item.assessment_id} but the submission is for ` +
                    `${submission.assessment_id}`, response.response_id);
            }
            if (response.student_id !== submission.student_id) {
                issues.error("response.student_mismatch", `response is for ${response.student_id} but ${response.submission_id} belongs to ` +
                    `${submission.student_id}`, response.response_id);
            }
        }
        if (item !== undefined) {
            if (response.score != null && response.score > item.maximum_score) {
                issues.error("score.out_of_range", `score ${f(response.score)} exceeds item maximum ${f(item.maximum_score)}`, response.response_id);
            }
            const labels = new Set((item.options ?? []).map((o) => o.label));
            for (const chosen of response.chosen_options ?? []) {
                if (labels.size && !labels.has(chosen)) {
                    issues.error("ref.option", `option '${chosen}' is not defined on ${item.item_id}`, response.response_id);
                }
            }
        }
    }
...
const checkRuntime = (b, issues) => {
    ...
    const runs = runById(b);
    const users = userById(b);
    const evidenceIds = new Set(b.evidence.map((e) => e.evidence_id));
    const signalIds = new Set(b.signals.map((s) => s.signal_id));
    const eventIds = new Set(b.events.map((e) => e.event_id));
    for (const submission of b.submissions) {
        if (!assessments.has(submission.assessment_id)) {
            issues.error("ref.assessment", `unknown assessment ${submission.assessment_id}`, submission.submission_id);
        }
    }
    for (const evaluation of b.evaluations) {
        if (!submissions.has(evaluation.submission_id)) {
            issues.error("ref.submission", `unknown submission ${evaluation.submission_id}`, evaluation.evaluation_id);
        }
        if (!criteria.has(evaluation.criterion_id)) {
            issues.error("ref.criterion", `unknown criterion ${evaluation.criterion_id}`, evaluation.evaluation_id);
        }
        else {
            const maximum = criteria.get(evaluation.criterion_id).maximum_score;
            for (const [label, side] of [
                ...
                if (side != null && side.score > maximum) {
                    issues.error("score.out_of_range", `${label} score ${f(side.score)} exceeds criterion maximum ${f(maximum)}`, evaluation.evaluation_id);
                }
            }
        }
        ...
        if (decided && decision == null) {
            issues.error("evaluation.unapproved", `status is ${evaluation.status} but there is no professor decision`, evaluation.evaluation_id);
        }
        else if (decided) {
            ...
            const missing = [
                ...
                .map(([name]) => name);
            if (missing.length) {
                issues.error("evaluation.unstamped", `status is ${evaluation.status} but the decision has no ${missing.join(" or ")} — ` +
                    "approve it with `ainar approve` rather than editing by hand", evaluation.evaluation_id);
            }
        }
        else if (decision != null) {
            issues.warn("evaluation.pending_override", "a professor decision is present but the evaluation is not approved yet", evaluation.evaluation_id);
        }
    ...
    for (const item of b.evidence) {
        if (!runs.has(item.course_version_id)) {
            issues.error("ref.course_run", `unknown course_version_id ${item.course_version_id}`, item.evidence_id);
        }
        if (item.outcome_id && !outcomes.has(item.outcome_id)) {
        ...
        if (!item.outcome_id && !item.capability_id && !item.concept_id) {
            issues.error("evidence.untargeted", "evidence must name an outcome, capability or concept", item.evidence_id);
        }
    }
    for (const state of b.concept_states) {
    ...
    for (const state of b.capability_states) {
        ...
        if (state.level != null && !(state.evidence_ids ?? []).length) {
            issues.error("capability.unevidenced", "a capability level with no evidence behind it is an assertion, not an estimate", where);
        }
        const capability = capabilities.get(state.capability_id);
        if (capability && (capability.levels ?? []).length && state.level != null) {
            const highest = Math.max(...capability.levels.map((level) => level.level));
            if (state.level > highest) {
                issues.error("capability.level_out_of_range", `level ${state.level} exceeds the ${highest} levels defined for ${state.capability_id}`, where);
            }
        }
    }
    for (const signal of b.signals) {
        if (!runs.has(signal.course_version_id)) {
            issues.error("ref.course_run", `unknown course_version_id ${signal.course_version_id}`, signal.signal_id);
        }
        for (const reference of signal.evidence_ids ?? []) {
            if (!evidenceIds.has(reference)) {
                issues.error("ref.evidence", `unknown evidence ${reference}`, signal.signal_id);
            }
        }
        if (!(signal.evidence_ids ?? []).length) {
            issues.error("signal.no_evidence", "a signal without evidence is an unexplained label", signal.signal_id);
        }
    }
    for (const intervention of b.interventions) {
        if (!runs.has(intervention.course_version_id)) {
            issues.error("ref.course_run", `unknown course_version_id ${intervention.course_version_id}`, intervention.intervention_id);
        }
        if (intervention.signal_id && !signalIds.has(intervention.signal_id)) {
            issues.error("ref.signal", `unknown signal ${intervention.signal_id}`, intervention.intervention_id);
        }
        if (["approved", "scheduled", "completed"].includes(intervention.status) &&
            !intervention.approved_by) {
            issues.error("intervention.unapproved", `status is ${intervention.status} but approved_by is empty`, intervention.intervention_id);
        }
    }
    for (const event of b.events) {
        if (!runs.has(event.course_version_id)) {
            issues.error("ref.course_run", `unknown course_version_id ${event.course_version_id}`, event.event_id);
        }
    }
    for (const action of b.action_items) {
        if (!runs.has(action.course_version_id)) {
            issues.error("ref.course_run", `unknown course_version_id ${action.course_version_id}`, action.action_id);
        }
...
const checkNoDrafts = (b, issues) => {
    for (const [collection, field] of ID_FIELDS) {
        for (const record of b[collection]) {
            if (String(record[field]).includes("-DRAFT-")) {
                issues.warn("id.unapproved_draft", `${collection}: draft identifier in the course record — approve it with ` +
                    "`ainar approve` rather than copying it", record[field]);
            }
        }
    }
    for (const [name, field] of [
        ...
        for (const record of b[name]) {
            const extensions = record.extensions ?? {};
            if ("proposal" in extensions && !("approval" in extensions)) {
                issues.warn("claim.unapproved_proposal", `${name.slice(0, -1)} was proposed by an agent but carries no approval stamp — ` +
                    "promote it with `ainar approve` rather than copying it", record[field]);
            }
        }
    }
};
export const validate = (b, options = {}) => {
    ...
    if (!options.draftsMerged)
        checkNoDrafts(b, issues);
    return issues;
};
/** The fixture shape: issues sorted the way `ainar golden` sorts them. */
export const validatePayload = (b) => ({
    issues: [...validate(b).items].sort((a, c) => a.code.localeCompare(c.code) ||
