...
export function validateLmsExam(value) {
    if (!value || typeof value !== "object")
        throw new Error("Exam input must be an object");
    const exam = value;
    ...
    for (const [index, question] of exam.questions.entries()) {
        ...
        if (!question.prompt?.trim())
            throw new Error(`Question ${question.number} has no prompt`);
        if (question.marks !== undefined && (!Number.isFinite(question.marks) || question.marks <= 0))
            throw new Error(`Question ${question.number} marks must be positive`);
        if (question.options?.length) {
            if (!question.correctOptions?.length)
                throw new Error(`Question ${question.number} requires correctOptions for LMS export`);
            if (question.correctOptions.length !== 1)
                throw new Error(`Question ${question.number} currently supports exactly one correct option`);
            const labels = new Set(question.options.map((option) => option.label));
            if (!labels.has(question.correctOptions[0]))
                throw new Error(`Question ${question.number} correct option does not match an option label`);
        }
    }
    return exam;
}
export function moodleXml(exam) {
    const questions = exam.questions.map((question) => {
        ...
        if (question.options?.length) {
            ...
            for (const option of question.options) {
                ...
                lines.push(`    <answer fraction="${correct}" format="html">`, `      <text>${cdata(`<p>${xml(option.text)}</p>`)}</text>`, "      <feedback format=\"html\"><text></text></feedback>", "    </answer>");
            }
        }
        else {
            lines.push("    <responseformat>editor</responseformat>", "    <responserequired>1</responserequired>", "    <responsefieldlines>15</responsefieldlines>", "    <attachments>0</attachments>", "    <answer fraction=\"0\"><text></text></answer>");
        }
        lines.push("  </question>");
        return lines.join("\n");
    });
...
export function qtiAssessmentXml(exam, assessmentId = "professorharness_exam") {
    return [
...
function qtiItem(question) {
    ...
    if (question.options?.length) {
        ...
        for (const option of question.options)
            lines.push(`            <response_label ident="${xml(option.label)}"><material><mattext texttype="text/plain">${xml(option.text)}</mattext></material></response_label>`);
        lines.push("          </render_choice></response_lid>", "        </presentation>", "        <resprocessing>", `          <outcomes><decvar maxvalue="${score}" minvalue="0" varname="SCORE" vartype="Decimal"/></outcomes>`, "          <respcondition continue=\"No\"><conditionvar>", `            <varequal respident="response">${xml(question.correctOptions?.[0] ?? "")}</varequal>`, "          </conditionvar>", `          <setvar action="Set" varname="SCORE">${score}</setvar></respcondition>`, "        </resprocessing>");
    }
    else {
        lines.push("          <response_str ident=\"response\" rcardinality=\"Single\"><render_fib fibtype=\"String\" rows=\"12\" columns=\"80\"><response_label ident=\"answer\" rshuffle=\"No\"/></render_fib></response_str>", "        </presentation>", "        <resprocessing>", `          <outcomes><decvar maxvalue="${score}" minvalue="0" varname="SCORE" vartype="Decimal"/></outcomes>`, "        </resprocessing>");
    }
    lines.push("      </item>");
    return lines.join("\n");
}
export function qtiManifestXml(exam, assessmentFile = "assessment.xml") {
    return [
        "<?xml version=\"1.0\" encoding=\"UTF-8\"?>",
...
export function exporterFor(target) {
    if (target === "moodle-xml")
        return { target, extension: ".xml", export: (exam) => ({ mediaType: "application/xml", data: encoder.encode(moodleXml(exam)) }) };
