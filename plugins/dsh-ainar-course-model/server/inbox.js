...
export const inboxPayload = (b, courseVersionId, on, options = {}) => {
    ...
    for (const evaluation of pending) {
        ...
        if (confidence !== null && confidence < LOW_CONFIDENCE) {
            entry.low_confidence = entry.low_confidence + 1;
        }
    }
    for (const entry of byAssessment.values()) {
        const assessment = assessmentById(b).get(entry.assessment_id);
        entry.title = assessment ? assessment.title : "";
    }
    const lowConfidence = pending
        .filter((evaluation) => evaluation.ai_suggestion &&
    ...
    const assessmentState = assessments.map((assessment) => {
        ...
        if (due === null)
            ...
            status = "closed";
        else if (daysBetween(on, due) <= DUE_SOON_DAYS)
            status = "due_soon";
        else
            status = "upcoming";
        return {
    ...
    return {
        ...
        pending_evaluations: {
            ...
            by_assessment: [...byAssessment.values()].sort((a, c) => c.pending - a.pending),
