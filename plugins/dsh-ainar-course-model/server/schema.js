...
import { Capability, Concept, ConceptEdge, Course, LearningOutcome, Module, } from "./model/academic.js";
...
export const SCHEMA_NAMES = Object.keys(SCHEMA_MODELS).sort();
/**
...
const innerOf = (schema) => schema?._def?.innerType ?? schema?._def?.schema ?? schema?._def?.type;
/**
    ...
    switch (kind) {
        ...
        case "number": {
            ...
            const isInt = checks.some((c) => c?.kind === "int") ||
                checks.some((c) => c?._zod?.def?.format === "safeint");
        ...
        case "union": {
            ...
            return { type: options.map((o) => describe(unwrap(o).inner).type).join(" or ") };
        }
        case "unknown":
        case "any":
            return { type: "any" };
...
export const shapeText = (name) => {
    const fields = shapeOf(name);
    if (!fields)
        return `${name} is not an entity. Try one of: ${SCHEMA_NAMES.join(", ")}`;
    const lines = [`${name}:`];
    for (const field of fields) {
        const parts = [field.type];
        if (field.pattern)
            parts.push(`matching /${field.pattern}/`);
        if (field.values)
            parts.push(`one of ${field.values.join(", ")}`);
        if (field.default !== undefined)
            parts.push(`default ${field.default}`);
        lines.push(`  ${field.required ? "*" : " "} ${field.name.padEnd(24)} ${parts.join(" · ")}`);
    }
    lines.push("");
...
export const jsonSchemaFor = (name) => {
    const model = SCHEMA_MODELS[name];
    if (!model)
        return null;
