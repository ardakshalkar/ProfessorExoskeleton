 ...
 */
import { allRubrics, assessmentsOf, conceptById, itemById, moduleById, modulesOf, outcomesOf, runById, } from "./bundle.js";
 ...
 */
export const outcomeGradeShare = (b, courseVersionId) => {
    const share = new Map();
    for (const assessment of assessmentsOf(b, courseVersionId)) {
        ...
        if (!rubric || !rubric.criteria.length)
            continue;
        const total = rubric.criteria.reduce((sum, c) => sum + c.maximum_score, 0) ||
...
const scopeWeek = (b, through) => {
    if (through === null || through === undefined)
        return null;
    if (/^\d+$/.test(through))
        return Number.parseInt(through, 10);
    const module = moduleById(b).get(through);
    return module ? (module.week ?? null) : null;
};
const push = (table, key, value) => {
    table.set(key, [...(table.get(key) ?? []), value]);
};
export const blueprintPayload = (b, courseVersionId, options = {}) => {
    ...
    const allOutcomes = new Map(outcomesOf(b, b.course.course_id).map((o) => [o.outcome_id, o]));
    const concepts = conceptById(b);
    const share = outcomeGradeShare(b, courseVersionId);
    const lastWeek = scopeWeek(b, through);
    const scopeModules = modulesOf(b, b.course.course_id).filter((module) => lastWeek === null || (module.week ?? 0) <= lastWeek);
    const scopeConcepts = [];
    const taughtIn = new Map();
    const moduleOutcomes = new Map();
    for (const module of scopeModules) {
        for (const conceptId of module.concepts) {
            if (!scopeConcepts.includes(conceptId))
                scopeConcepts.push(conceptId);
            push(taughtIn, conceptId, module.module_id);
        }
        for (const outcomeId of module.outcomes)
            push(moduleOutcomes, outcomeId, module.module_id);
    }
    ...
    for (const rubric of allRubrics(b).values()) {
        for (const criterion of rubric.criteria ?? []) {
            for (const conceptId of criterion.concepts)
                push(assessedBy, conceptId, criterion.criterion_id);
        }
    }
    ...
    for (const response of b.item_responses) {
        ...
        if (!item || response.score === null || response.score === undefined)
            continue;
    ...
    const weakStates = new Set(b.concept_states
        .filter((state) => state.course_version_id === courseVersionId && ["developing", "needs_review"].includes(state.state))
        .map((state) => state.concept_id));
    const signalled = new Set(b.signals
        .filter((signal) => signal.course_version_id === courseVersionId && signal.status === "open")
        .flatMap((signal) => signal.concepts));
    // Suggested mark allocation, proportional to declared outcome weight.
    const weights = new Map(targetIds.map((id) => [id, allOutcomes.get(id).weight ?? 0]));
    const totalWeight = [...weights.values()].reduce((a, b2) => a + b2, 0);
    const allocation = new Map([...weights.entries()].map(([id, value]) => [
        id,
    ...
    return {
        ...
        target_outcomes: targetIds.map((outcomeId) => {
            const outcome = allOutcomes.get(outcomeId);
            return {
