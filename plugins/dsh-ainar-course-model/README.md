...
in `node/src/mcp/tools.ts` rather than a promise made by this bundle.

Approval stays where it always is: a person running `ainar approve`.

## Generated

`index.js` and `server/` are built by `python plugin/build.py --target dsh`.
Edit `index.ts` and rebuild; do not edit the bundle.
